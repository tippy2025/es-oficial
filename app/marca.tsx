...
export function Escudo({ className = "", conPregunta = true }: { className?: string; conPregunta?: boolean }) {
  return (
...
export function Marca({ className = "" }: { className?: string }) {
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      <Escudo className="h-8 w-8 text-[var(--brand)]" />
      <span className="text-lg font-bold tracking-tight text-[var(--brand)]">¿Es Oficial?</span>
    </div>
  );
}
